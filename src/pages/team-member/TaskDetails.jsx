...
function TaskDetails() {
  ...
  const getMemberID = () => {
    fetch(`https://localhost:8080/api/members/`)
      ...
      .then((data) => {
        const miembrosFiltrados = data.filter(
          (miembro) => miembro.id_usuario === idUser
        );
  ...
  const getTask = async () => {
    try {
      const response = await fetch(
        `https://localhost:8080/api/task/${idNumerico}`
      );
      ...
      if (data.length > 0 && idMiembro !== null) {
        ...
        const taskWithDifferentId = data.find(
          (task) => task.id_miembro_id !== idMiembro
        );
        ...
        <div className="flex flex-col user-info mx-5 md:mx-10">
          ...
          <div className="flex bg-[#8DA8C5] rounded-md my-4 mx-24 h-[550px] w-[1300px]">
            ...
            <div className="w-1/4 bg-[#8DA8C5] rounded-md">
              <div className="mt-6 flex flex-col  mr-4 items-center justify-center">
                {" "}
                {/* Ajuste aquí */}
                <img
                  src={options}
